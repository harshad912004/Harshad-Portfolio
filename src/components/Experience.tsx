...
const Experience: React.FC = () => {
  return (
    <section id="experience" className="py-24 px-6 md:px-10 bg-[var(--background)] border-t border-[var(--border-slate)]/40">
      <div className="max-w-6xl mx-auto">
        ...
        >
          <h2 className="text-3xl md:text-5xl font-extrabold font-heading text-[var(--foreground)] mb-4">Work Experience of Harshad Dhongade</h2>
          <div className="w-20 h-1.5 bg-[var(--accent-coral)] mx-auto rounded-full"></div>
        </motion.div>

        <div className="relative border-l-2 border-[var(--border-slate)] ml-4 md:ml-6 pl-8 space-y-12">
          {experienceData.map((exp, index) => (
            ...
            >
              ...
              <div className="absolute -left-[41px] md:-left-[43px] top-9 w-5 h-5 bg-[var(--accent-coral)] rounded-full border-4 border-[var(--background)] group-hover:scale-125 transition-transform duration-300"></div>
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-2">
                <div>
                  ...
                  <h4 className="text-lg text-[var(--foreground)] font-heading font-semibold mt-1">{exp.company}</h4>
                </div>
                <div className="flex flex-col md:items-end text-[var(--foreground)]/70 text-sm gap-1">
                  <span className="flex items-center"><Calendar size={14} className="mr-1.5 text-[var(--accent-coral)]" /> {exp.period}</span>
                  <span className="flex items-center"><MapPin size={14} className="mr-1.5 text-[var(--accent-coral)]" /> {exp.location}</span>
                </div>
              </div>

              <p className="text-[var(--foreground)]/80 leading-relaxed mb-6 font-medium">
                {exp.description}
              </p>
              ...
              <ul className="space-y-3 mb-8">
                {exp.points.map((pt, i) => (
                  <li key={i} className="flex items-start gap-3 text-sm text-[var(--foreground)]">
                    <CheckCircle2 size={16} className="text-[var(--accent-coral)] mt-0.5 flex-shrink-0" />
              ...
              <div className="flex flex-wrap gap-2">
                {exp.tech.map((t) => (
                  <span
                    key={t}
                    className="px-3 py-1 bg-[var(--bg-card)] border border-[var(--border-slate)]/50 rounded-md text-xs font-mono text-[var(--foreground)]"
                  >
                    {t}
                  </span>
                ))}
